...
TCustomCBSDBGrid = class(TCustomDBGrid)
private
...
function TCustomCBSDBGrid.BuildFilter: String;
 var filter:String;
begin
  filter := 'patunique = '+IntToStr(Patient);
  if FCurrentExamOnly then
    filter := filter + ' and examunique = '+IntToStr(Exam);
  if FFilterStatement.Text<>'' then
    filter := filter + ' and ' + FFilterStatement.Text;
  Result := filter;
end;
...
{  Change filtering to not be part of the SQL but to be a filter
...
{ Change because of filtering change
  ...
  begin
  Close;
//  if FindField('patunique')<>nil then
//     Patient := FieldByName('patunique').AsInteger;
//   if FindField('examunique')<>nil then
//         Exam := FieldByName('examunique').AsInteger;
   filter := BuildFilter;
   filtered := true;
  Open;
  RecNo := FLinkedDataLink.DataSet.RecNo;
  end;
end;
